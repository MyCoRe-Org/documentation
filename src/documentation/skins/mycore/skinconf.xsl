...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  <xsl:template match="skinconfig">
    <xsl:if test="not(colors)">
      <colors>
<!-- Lenya -->
        <color name="header" value="#E5E4D9"/>
        <color name="tab-selected" value="#ffffff" link="#000000" vlink="#000000" hlink="#000000"/>
        <color name="tab-unselected" value="#4c6c8f" link="#dddddd" vlink="#dddddd" hlink="#dddddd"/>
        ...
        <color name="published" value="#15516E" font="#FFFFFF"/>
        <color name="feedback" value="#4C6C8F" font="#FFFFFF" align="center"/>
        <color name="navstrip" value="#15516E" font="#000000"/>
        <color name="toolbox" value="#15516E" font="orange"/>
        <color name="border" value="#999999"/>
        <color name="menu" value="#4C6C8F" font="#ffffff" link="#ffffff" vlink="#ffffff" hlink="#ffffff"  />
        <color name="menuheading" value="#e2e6e9" font="#000000" />
        <color name="searchbox" value="#15516E" font="#000000"/>
        ...
        <color name="footer" value="#E5E4D9"/>    </colors>
    </xsl:if>
    <xsl:copy>
      <xsl:copy-of select="@*"/>
  ...
  <xsl:template match="colors">
    <xsl:copy>
      ...
      <xsl:if test="not(color[@name='header'])">
        <color name="header" value="#15516E"/>
      </xsl:if>
      <xsl:if test="not(color[@name='tab-selected'])">
        <color name="tab-selected" value="#ffffff" link="#000000" vlink="#000000" hlink="#000000"/>
      </xsl:if>
      <xsl:if test="not(color[@name='tab-unselected'])">
        <color name="tab-unselected" value="#dddddd" link="#666666" vlink="#666666" hlink="#666666"/>
      </xsl:if>
      ...
      <xsl:if test="not(color[@name='heading'])">
        <color name="heading" value="#15516E"/>
      </xsl:if>
      <xsl:if test="not(color[@name='subheading'])">
        <color name="subheading" value="#15516E"/>
      </xsl:if>
      <xsl:if test="not(color[@name='feedback'])">
        <color name="feedback" value="#15516E" font="#FFFFFF" align="center"/>
      </xsl:if>
      <xsl:if test="not(color[@name='published'])">
        <color name="published" value="#15516E" font="#FFFFFF"/>
      </xsl:if>
      <xsl:if test="not(color[@name='navstrip'])">
        <color name="navstrip" value="#15516E" font="#FFFFFF"/>
      </xsl:if>
      ...
      <xsl:if test="not(color[@name='menu'])">
        <color name="menu" value="#15516E" font="#ffffff" link="#ffffff" vlink="#ffffff" hlink="#ffffff"  />
      </xsl:if>
      ...
      <xsl:if test="not(color[@name='footer'])">
        <color name="footer" value="#15516E"/>
      </xsl:if>
    </xsl:copy>
  </xsl:template>
</xsl:stylesheet>
