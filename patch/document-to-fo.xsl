...
<!-- Emulate browser handling of these invalid combinations that our DTD
  ...
  <xsl:template match="dl[@class='kasten']">
    <fo:block
      border="0.5pt solid"   
      padding="5pt"  
...
<!-- Make relative paths absolute -->
      <xsl:variable name="imgpath">
        <xsl:variable name="mcrimagesdir">http://www.mycore.de/documentation/images</xsl:variable>
        <xsl:choose>
<!-- resources image dir -->
          <xsl:when test="starts-with(string(@src),'images/')">
            <xsl:value-of select="concat($mcrimagesdir,substring-after(@src,'images'))"/>
          </xsl:when>
...
<!-- ====================================================================== -->
  ...
  <xsl:template match="p[@class='fett']">
    <fo:block 
     space-before="8pt"          
     font-weight="bold">
      <xsl:call-template name="insertPageBreaks"/>
  ...
  <xsl:template match="p[@class='break']">
    <fo:block 
     space-before="4pt"        
     space-after="4pt"  
     start-indent="1.5em">
  ...
  <xsl:template match="p[@class='kastensource']">
    <fo:block 
    border="0.5pt solid"     
     background-color="#E2F2F8"
     space-before="6pt"
  ...
  <xsl:template match="p[@class='kasten']">
    <fo:block 
     border="0.5pt solid"   
     padding="4pt"  
     background-color="#E2F2F8"
  ...
  <xsl:template match="p[@style='color:red']">    
    <fo:block
      color="red" 
      space-before="4pt" 
      space-after="4pt">
